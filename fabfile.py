...
monit-graphite is a fabric script to install latest monit (5.11) on debian hosts
...
def monit_install(force=False):
    ...
    with settings(warn_only=True):
        result = sudo('monit -V')
        if result.succeeded and "Monit version 5.11" in result and not force:
            print "Monit 5.11 already installed. Skipping."
    ...
    with cd('/usr/local/src'):
        sudo('wget -N http://mmonit.com/monit/dist/monit-5.11.tar.gz')
        sudo('tar -zxvf monit-5.11.tar.gz')
    with cd('/usr/local/src/monit-5.11'):
        with settings(warn_only=True):
